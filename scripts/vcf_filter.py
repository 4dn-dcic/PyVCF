...
from vcf.parser import (
    _Filter,  # noQA - testing protected member
)
...
def main():
    ...
    while len(args.rest):
        ...
        if filter_name not in filters:
            sys.exit(f"{filter_name} is not a known filter. Options are: {', '.join(filters.keys())}")
        ...
        if len(unknown_filt_args):
            sys.exit(f"{filter_name} has no arguments like: {', '.join(unknown_filt_args)}")
